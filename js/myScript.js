//05-sample-1 mySCript.js
class Main extends Phaser.Scene {
  constructor() {
    super('Main');
  }
  preload() {
    this.load.image('bg', 'asset/bg/bgresized.png');
  ...
  create() {
    ...
    */
    ...
    // 赤ちゃん天使
    // TODO: 当たり判定をいい感じにする
    this.littleangels = this.physics.add.sprite(50, height-750, 'littleangel');
    this.littleangels.setBounceY(Phaser.Math.FloatBetween(0.4, 0.8));
    // プレイヤー
    ...
    this.player.setSize(130, 130, 1, 1);
    ...
    */
    ...
    //this.blocks.setSize(100, 100, 1, 1);
    // 敵の悪魔
    ...
    let genEnemy = ()=>{
      ...
      */
      ...
      var enemy = this.physics.add.sprite(this.player.x, this.player.y, 'demon');
      enemy.setSize(100, 100, 1, 1);
      enemy.setBounce(0.8);
    ...
    this.physics.add.overlap(this.player, this.enemy, gameOver, null, this);
    //クリア処理
...
class GameClear extends Phaser.Scene {
  ...
  preload() {
    // TODO: 背景素材の差し替え
    this.load.audio('gameClearSound', 'asset/sounds/gameclear1.mp3');
    this.load.image('gameClearImg', 'asset/scene/gameclear1.png');
  }
  create() {
    this.gameOverSound = this.sound.add('gameClearSound');
    this.gameOverSound.play();
    this.spaceKey = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SPACE);
    this.gameClearImg = this.add.image(400, 300, 'gameClearImg');
    this.exprainText = this.add.text(180, 400, 'RESTART : Press SPACE KEY', {
      fontSize: '32px',
      fill: '#ffffff'
    });
